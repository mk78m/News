...
async function fetchSource(source){
  ...
  const alternates=alternateFeedCandidates(source);
  for(const u of alternates) if(!feedUrls.includes(u)) feedUrls.push(u);
  // For the core sources, direct RSS is the first-class transport. This keeps the server
  // fast and prevents dozens of proxy requests from starving otherwise healthy sources.
  const direct=feedUrls.slice(0,10).map(u=>parseFeed(u,source).then(items=>({items,method:'rss',sourceUrl:u})));
  const directResults=await Promise.allSettled(direct);
  const merged=[]; const seen=new Set();
  const add=(items)=>{for(const item of (items||[])){const k=canonicalUrl(item.link)||item.id;if(k&&!seen.has(k)){seen.add(k);merged.push(item);}}};
  for(const r of directResults) if(r.status==='fulfilled' && r.value?.items?.length) add(r.value.items);
  if(merged.length){
    merged.sort((a,b)=>(b.published_ts||0)-(a.published_ts||0));
    sourceCache.set(source.id,{items:merged.slice(0,80),ts:Date.now()});
    return {items:merged.slice(0,80),method:'rss-direct-parallel',sourceUrl:feedUrls.join(' | ')};
  }
  // Fallbacks are only used when the direct feed paths fail, so one blocked endpoint
  // cannot consume the connection/time budget of all other sources.
  const fallback=[];
  if(cfg.page) fallback.push(parseHtml(cfg.page,source,cfg).then(items=>({items,method:'site-html',sourceUrl:cfg.page})));
  for(const gn of googleNewsFeedsFor(source)) fallback.push(parseFeed(gn,source).then(items=>({items,method:'google-news',sourceUrl:gn})));
  if(cfg.page) fallback.push(fetchViaJina(cfg.page,source).then(items=>({items,method:'jina-reader',sourceUrl:cfg.page})));
  const fr=await Promise.allSettled(fallback);
  for(const r of fr) if(r.status==='fulfilled' && r.value?.items?.length) add(r.value.items);
  if(merged.length){
    merged.sort((a,b)=>(b.published_ts||0)-(a.published_ts||0));
    sourceCache.set(source.id,{items:merged.slice(0,80),ts:Date.now()});
    return {items:merged.slice(0,80),method:'fallback-parallel',sourceUrl:cfg.page};
  }
