...
const SOURCES = {
  ...
  cnn:{page:'https://edition.cnn.com/world', feeds:['https://rss.cnn.com/rss/edition_world.rss'], items:['article','.container__item','.card'], title:['.container__headline-text','h3','h2'], link:['a[href]'], desc:['.container__description','p'], image:['img'], date:['time[datetime]','time']},
  ...
  hindu:{page:'https://www.thehindu.com/news/international/', feeds:['https://www.thehindu.com/news/international/feeder/default.rss'], items:['article','li','.card','.teaser'], title:['h2','h3','h4','.title'], link:['a[href]'], desc:['p','.description','.summary'], image:['img'], date:['time[datetime]','time']},
...
const SOURCE_STRATEGIES = {
  // هر منبع فقط مسیر اختصاصی خودش را دارد. RSS رسمی اولویت اول است.
  bbc:['rss'], cnn:['rss','html'], aljazeera:['rss','html'], guardian:['rss'], skynews:['rss'], nbc:['rss'],
  euronews:['rss'], france24:['rss'], cbc:['rss'], npr:['rss'], rfi:['rss'], upi:['rss'], pbs:['rss'], cbs:['rss'],
  abcau:['rss'], hindu:['rss'], time:['rss'],
  nyt:['rss'], dw:['rss'], nhk:['rss'], smh:['rss'], indianexpress:['rss'], scmp:['rss'], lemonde:['rss'],
  politico:['rss'], techcrunch:['rss'], verge:['rss'], arstechnica:['rss'], nature:['rss'], spacenews:['rss'],
  sciencedaily:['rss'], economist:['rss'],

  // Reuters و ABC News فید عمومی رسمی پایدار در دسترس ندارند؛ query دامنه‌ای Google News
  // فقط برای همین دو منبع استفاده می‌شود و HTML خود همان سایت پشتیبان اختصاصی است.
  reuters:['google','html'], abc:['google','html'],

  // منابع فارسی: فید رسمی خود ناشر؛ بدون زنجیره proxy عمومی.
  irna:['rss'], isna:['rss'], mehr:['rss'], tasnim:['rss'], fars:['rss'], khabaronline:['rss'],
  entekhab:['rss'], etemadonline:['rss'], jamaran:['rss'], asriran:['rss'], tabnak:['rss'], mizanonline:['rss'], iranintl:['rss']
};
...
app.get('/api/feed-status',(_,res)=>res.json({ok:true,time:new Date().toISOString(),sources:Object.keys(SOURCES).length,cachedFeeds:feedCache.size,sourceStrategies:SOURCE_STRATEGIES,profiles:Object.fromEntries(Object.entries(SOURCES).map(([id,x])=>[id,{feeds:x.feeds,page:x.page,strategies:SOURCE_STRATEGIES[id]||['rss']}]))}));
...
app.get('/api/source-config',(_,res)=>res.json({ok:true,sources:Object.entries(SOURCES).map(([id,x])=>({id,page:x.page,feeds:x.feeds,strategies:SOURCE_STRATEGIES[id]||['rss'],cached:Boolean(sourceCache.get(id)),cachedItems:sourceCache.get(id)?.items?.length||0}))}));
