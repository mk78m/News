...
const ALL_STRATEGIES = ['rss','discover','html','jina','sitemap','google','proxyfeed','proxyhtml'];
...
async function fetchSource(source){
  ...
  if(enabled.has('rss')) for(const feed of (cfg.feeds||[])) push('rss',feed);
...
app.get('/api/feed-status',(_,res)=>res.json({ok:true,time:new Date().toISOString(),sources:Object.keys(SOURCES).length,cachedFeeds:feedCache.size,strategies:['rss','autodiscovery','html','jina-reader','sitemap','google-news','proxyfeed','proxyhtml','jina-feed'],sourceStrategies:Object.fromEntries(Object.keys(SOURCES).map(id=>[id,SOURCE_STRATEGIES[id]||['rss','discover','html','jina','sitemap','google']]))}));
