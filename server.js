...
async function fetchSource(source){const cfg=SOURCES[source.id];if(!cfg)throw new Error('Unknown source: '+source.id);const errors=[];
  for(const feed of cfg.feeds){try{const items=await parseFeed(feed,source);if(items.length)return {items,method:'rss',sourceUrl:feed}}catch(e){errors.push(`rss:${e.message}`)}}
  try{const items=await parseHtml(cfg.page,source,cfg);if(items.length)return {items,method:'site-html',sourceUrl:cfg.page}}catch(e){errors.push(`html:${e.message}`)}
...
app.get('/api/feed-status',(_,res)=>res.json({ok:true,time:new Date().toISOString(),sources:Object.keys(SOURCES).length,cachedFeeds:feedCache.size,cacheHits:[...feedCache.values()].length}));
...
setInterval(()=>backgroundPoll().catch(()=>{}),60*1000);
