...
const SOURCE_STRATEGIES = {
  bbc:['rss','discover','html','google'], cnn:['rss','discover','html','google'], reuters:['rss','discover','html','google'],
  aljazeera:['rss','discover','html','google'], guardian:['rss','discover','html','google'], nyt:['rss','discover','html','google'],
  dw:['rss','discover','html','google'], nbc:['rss','discover','html','google'], france24:['rss','discover','html','google'],
  euronews:['rss','discover','html','google'], nhk:['rss','discover','html','google'], scmp:['rss','discover','html','google'],
  techcrunch:['rss','discover','html','google'], verge:['rss','discover','html','google'], nature:['rss','discover','html','google']
};
...
function googleNewsFeedFor(source){
...
async function fetchSource(source){
  ...
  const candidates=[];
  const enabled=new Set(SOURCE_STRATEGIES[source.id]||['rss','discover','html','google']);
  ...
  const gn=googleNewsFeedFor(source); if(enabled.has('google') && gn) candidates.push({kind:'google',url:gn});
...
app.get('/api/feed-status',(_,res)=>res.json({ok:true,time:new Date().toISOString(),sources:Object.keys(SOURCES).length,cachedFeeds:feedCache.size,strategies:['rss','autodiscovery','html','google-news'],sourceStrategies:Object.fromEntries(Object.keys(SOURCES).map(id=>[id,SOURCE_STRATEGIES[id]||['rss','discover','html','google']]))}));
