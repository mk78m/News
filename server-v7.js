...
const SOURCE_STRATEGIES = {
  bbc:['rss','discover','html','jina','sitemap','google'], cnn:['rss','discover','html','jina','sitemap','google'], reuters:['rss','discover','html','jina','sitemap','google'],
  aljazeera:['rss','discover','html','jina','sitemap','google'], guardian:['rss','discover','html','jina','sitemap','google'], nyt:['rss','discover','html','jina','sitemap','google'],
  dw:['rss','discover','html','jina','sitemap','google'], nbc:['rss','discover','html','jina','sitemap','google'], france24:['rss','discover','html','jina','sitemap','google'],
  euronews:['rss','discover','html','jina','sitemap','google'], nhk:['rss','discover','html','jina','sitemap','google'], scmp:['rss','discover','html','jina','sitemap','google'],
  techcrunch:['rss','discover','html','jina','sitemap','google'], verge:['rss','discover','html','jina','sitemap','google'], nature:['rss','discover','html','jina','sitemap','google']
};
...
async function fetchOneStrategy(kind,url,source){
  if(kind==='rss') return await parseFeed(url,source);
  ...
  if(kind==='sitemap') { const maps=await discoverSitemaps(url); const rs=await Promise.allSettled(maps.map(m=>parseSitemap(m,source))); return rs.flatMap(x=>x.status==='fulfilled'?x.value:[]); }
  if(kind==='discover'){
...
async function fetchSource(source){
  ...
  const candidates=[];
  const enabled=new Set(SOURCE_STRATEGIES[source.id]||['rss','discover','html','jina','sitemap','google']);
  if(enabled.has('rss')) for(const feed of (cfg.feeds||[])) candidates.push({kind:'rss',url:feed});
  if(enabled.has('discover') && cfg.page) candidates.push({kind:'discover',url:cfg.page});
  if(enabled.has('html') && cfg.page) candidates.push({kind:'html',url:cfg.page});
  if(enabled.has('jina') && cfg.page) candidates.push({kind:'jina',url:cfg.page});
  if(enabled.has('sitemap') && cfg.page) candidates.push({kind:'sitemap',url:cfg.page});
  if(enabled.has('google')) for(const gn of googleNewsFeedsFor(source)) candidates.push({kind:'google',url:gn});
  // All independent transports start together. We merge every successful result instead of stopping at the first one.
  const results=await Promise.allSettled(candidates.map(c=>fetchOneStrategy(c.kind,c.url,source)));
  const merged=[]; const seen=new Set(); const methods=[]; const errors=[];
  results.forEach((r,i)=>{
    if(r.status==='fulfilled'){
      const arr=Array.isArray(r.value)?r.value:[];
      if(arr.length) methods.push(candidates[i].kind);
      for(const item of arr){ if(item && !seen.has(item.id)){seen.add(item.id);merged.push(item);} }
    }else errors.push(`${candidates[i].kind}:${r.reason?.message||'failed'}`);
  });
  merged.sort((a,b)=>b.published_ts-a.published_ts);
  if(!merged.length) throw new Error(errors.slice(0,6).join(' | ')||'No articles found');
...
app.get('/api/feed-status',(_,res)=>res.json({ok:true,time:new Date().toISOString(),sources:Object.keys(SOURCES).length,cachedFeeds:feedCache.size,strategies:['rss','autodiscovery','html','jina-reader','sitemap','google-news'],sourceStrategies:Object.fromEntries(Object.keys(SOURCES).map(id=>[id,SOURCE_STRATEGIES[id]||['rss','discover','html','jina','sitemap','google']]))}));
